...
- **Semantic HTML**: Renders proper heading tags (h1-h6) via the `as` prop
- **Color System**: Supports all Versaur color variants (primary, secondary, tertiary, ghost,
  neutral, success, info, warning, danger)
- **Typography Control**: Built-in typography styles
- **Text Styling**: Decoration (`underline`, `line-through`, `overline`), transform (`capitalize`,
  `uppercase`, `lowercase`), alignment options
...
<Heading as='h2' color='primary' hasMargin>
  Section Title
</Heading>
...
| ------------ | ------------------------------------------------------- | ----------- | ------------------------------- |
| `as`         | `'h1' \| 'h2' \| 'h3' \| 'h4' \| 'h5' \| 'h6'`          | `'h1'`      | Heading tag to render           |
| `color`      | `Color`                                                 | `'ghost'`   | Color from Versaur color system |
| `transform`  | `'none' \| 'capitalize' \| 'uppercase' \| 'lowercase'`  | `'none'`    | Text transform helper           |
| `decoration` | `'none' \| 'underline' \| 'line-through' \| 'overline'` | `'none'`    | Text decoration helper          |
